...
export default function BlogDashboard() {
  const { data, isLoading, error, refetch } = useQuery({ queryKey: ["blogAdmin", "dashboard"], queryFn: () => listBlogAdminData("dashboard") });
...
function Quick({ to, icon: Icon, label }) { return <Button variant="outline" asChild className="min-h-11 h-auto justify-start rounded-2xl p-4"><Link to={to}><Icon className="h-5 w-5" aria-hidden="true" />{label}</Link></Button>; }
