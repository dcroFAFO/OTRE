...
export default function BlogLogs() {
  const { data, isLoading, error, refetch } = useQuery({ queryKey: ["blogAdmin", "logs"], queryFn: () => listBlogAdminData("logs") });
  const logs = data?.logs || [];
  return (
    <div className="space-y-5">
      <BlogAdminHeader title="Blog Logs" description="Recent blog activity and publishing events." />
      {error && <ErrorState error={error} onRetry={refetch} />}
      {isLoading && <TableSkeleton rows={5} columns={3} label="Loading blog activity" />}
      {!isLoading && !error && (
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        ...
        {logs.length === 0 && <p className="p-8 text-center text-sm text-muted-foreground">No blog activity yet.</p>}
      </div>
      )}
    </div>
  );
