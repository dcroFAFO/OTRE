...
function Author({ post }) { return <div className="mt-10 flex gap-4 rounded-3xl border border-border bg-card p-5">{post.author_avatar_url && <img src={post.author_avatar_url} alt={post.author_name} className="h-14 w-14 rounded-full object-cover" />}<div><p className="font-semibold">{post.author_name || "OTR Scooters"}</p>{post.author_bio && <p className="text-sm text-muted-foreground">{post.author_bio}</p>}</div></div>; }