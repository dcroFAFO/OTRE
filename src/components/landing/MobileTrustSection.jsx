...
/** @type {Array<[React.ElementType, string, string]>} */
const FEATURES = [
    ...
    <section className="border-y border-border bg-card/55 px-4 py-14 sm:px-8 sm:py-20">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {FEATURES.map(([Icon, title, text]) => <article key={title} className="rounded-lg border border-border bg-card p-5"><Icon className="h-5 w-5 text-accent" aria-hidden="true" /><h3 className="mt-3 font-bold">{title}</h3><p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">{text}</p></article>)}
        </div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {REVIEWS.map(([quote, author]) => <figure key={author} className="rounded-lg border border-border bg-card p-5"><div className="flex gap-0.5" role="img" aria-label="5 out of 5 stars">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-4 w-4 fill-accent text-accent" aria-hidden="true" />)}</div><blockquote className="mt-3 text-sm leading-relaxed">“{quote}”</blockquote><figcaption className="mt-3 text-sm font-bold">{author}</figcaption></figure>)}
        </div>
      </div>
    </section>
  );
