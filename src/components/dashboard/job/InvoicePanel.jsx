...
export default function InvoicePanel({ job, actor, canEdit, onChange }) {
  ...
  // Build line items from quote labour + inventory usage
  const lineItems = [];
  ...
  const lineTotal = lineItems.reduce((s, li) => s + li.qty * li.unit_price, 0);
            ...
            <tbody>
              {lineItems.map((li, i) => (
                <tr key={i} className="border-b border-border/50 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-right text-muted-foreground">{currency} {li.unit_price.toFixed(2)}</td>
                  <td className="px-4 py-2.5 text-right font-medium">{currency} {(li.qty * li.unit_price).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
