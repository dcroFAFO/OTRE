...
export default function JobActions({ job, actor, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-heading font-bold">Manage job</h3>
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        <Button size="sm" variant="outline" className="gap-1.5" onClick={run(() => markReadyForPickup(job, actor))}><PackageCheck className="h-4 w-4" /> Ready for pickup</Button>
        <Button size="sm" variant="outline" className="gap-1.5" onClick={run(() => changeStatus(job, "completed", actor))}>Complete</Button>
