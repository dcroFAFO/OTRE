...
export default function PortalTutorial({ onDone }) {
      ...
      <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/30 p-4">
        <div className="w-full max-w-sm rounded-3xl border border-border bg-card p-6 text-center shadow-2xl">
          ...
          <div className="mt-5 flex justify-center gap-3">
            <Button variant="outline" onClick={finish}>Close</Button>
  ...
  if (step === 1) {
    return (
      <div className="fixed inset-0 z-50 bg-foreground/30 p-4">
  ...
  if (step >= 2 && step <= 4) {
    return <TutorialMockJobModal step={step} onNext={next} onClose={finish} />;
  }
