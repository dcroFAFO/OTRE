...
async function findOrCreateScooterForCustomer(
  entities,
  customer,
  ...
  const [byStable, byAccount] = await Promise.all([
    entities.Scooter.filter({ customer_id: stableId }, "-updated_date", 100)
      .catch(() => []),
  ...
  const scooters = [
    ...new Map([...byStable, ...byAccount].map((s) => [s.id, s])).values(),
  ];
  ...
  if (existing) {
    ...
    if (payload.odometer_km && !existing.odometer_km) {
      updates.odometer_km = payload.odometer_km;
    }
    return Object.keys(updates).length
      ? await entities.Scooter.update(existing.id, updates)
      : existing;
  }
  return await entities.Scooter.create({
    ...payload,
    customer_id: stableId,
    customer_account_id: accountId,
    job_id: jobId || "",
...
async function listScootersForCustomer(entities, customerId) {
  ...
  const phone = normalizePhone(
    customer.phone_e164 || customer.phone || customer.phone_display,
  );
  const [
    byStable,
    byAccount,
    ...
    entities.Scooter.filter({ customer_account_id: customer.id }, "make", 100)
      .catch(() => []),
  ...
  const jobs = [
    ...new Map(
      [...jobsByStable, ...jobsByAccount, ...jobsByEmail, ...jobsByPhone]
        .filter((job) => {
          if (
            job.customer_id && job.customer_id !== stableId &&
            job.customer_id !== customer.id
          ) return false;
          if (
            job.customer_account_id && job.customer_account_id !== customer.id
          ) return false;
          return true;
        }).map((job) => [job.id, job]),
    ).values(),
  ...
  return scooters.filter((scooter) => !scooter.archived_at).map((scooter) => {
    const label = [scooter.make, scooter.model].filter(Boolean).join(" ");
    const relatedJobs = jobs.filter((job) =>
      job.asset_id === scooter.id ||
      cleanText(
          job.asset_label || job.scooter_make_model || job.scooter_details,
        ) === cleanText(label)
    );
    ...
    return {
      ...scooter,
      related_job_count: relatedJobs.length,
...
async function saveScooter(entities, actor, payload) {
  ...
  const existing = payload.scooter_id
    ? await entities.Scooter.get(payload.scooter_id).catch(() => null)
    : null;
  ...
  const customerId = payload.customer_id || existing?.customer_account_id ||
    existing?.customer_id || "";
  if (!String(data.model || "").trim()) {
    throw new Error("Scooter model is required");
  }
  const customer = customerId
    ? await findCustomerRecord(entities, customerId)
    : null;
  ...
  const ownership = customer
    ...
    : {};
  let scooter;
  if (payload.scooter_id) {
    scooter = await entities.Scooter.update(payload.scooter_id, {
      ...data,
      ...ownership,
    });
  } else if (customer) {
    scooter = await findOrCreateScooterForCustomer(entities, customer, data);
  } else scooter = await entities.Scooter.create({ ...data });
  await logCustomerAudit(
    entities,
    actor,
    customer || { id: "" },
    `Scooter ${payload.scooter_id ? "updated" : "added"}: ${
...
async function removeScooter(entities, actor, payload) {
  ...
  const existing = await entities.Scooter.get(payload.scooter_id).catch(() =>
    null
  );
  if (!existing) throw new Error("Scooter not found");
  const customerId = payload.customer_id || existing.customer_account_id ||
    existing.customer_id || "";
  const linkedJobs = await entities.Job.filter(
  ...
  if (
    linkedJobs.length ||
    String(existing.job_id || "").split(",").filter(Boolean).length
  ) {
    throw new Error(
      "Linked scooters must be archived to preserve service history",
    );
  }
...
async function archiveScooter(entities, actor, payload) {
  if (!payload.scooter_id) throw new Error("scooter_id is required");
  const existing = await entities.Scooter.get(payload.scooter_id).catch(() =>
    null
  );
  if (!existing || existing.archived_at) throw new Error("Scooter not found");
  const linkedJobs = await entities.Job.filter(
    { asset_id: existing.id },
    "-created_date",
    1,
  ).catch(() => []);
  const hasJobs = linkedJobs.length > 0 ||
    String(existing.job_id || "").split(",").filter(Boolean).length > 0;
  if (!hasJobs) {
    throw new Error("Unlinked scooters can be deleted instead of archived");
  }
  const archived = await entities.Scooter.update(existing.id, {
    archived_at: new Date().toISOString(),
    archived_by_user_id: actor.id,
    archive_reason: cleanText(
      payload.reason || "Archived by administrator; service history retained",
    ).slice(0, 500),
  });
  await logCustomerAudit(
    entities,
    actor,
    { id: existing.customer_account_id || existing.customer_id || "" },
    "Scooter archived; service history retained",
    { scooter_id: existing.id },
  );
  return archived;
}
...
Deno.serve(async (req) => {
  try {
    ...
    if (action === "listScooters") {
      return Response.json({
        scooters: await listScootersForCustomer(entities, payload.customer_id),
      });
    }
    if (action === "saveScooter") {
      return Response.json({
        scooter: await saveScooter(entities, user, payload),
      });
    }
    if (action === "deleteScooter") {
      await removeScooter(entities, user, payload);
      return Response.json({ success: true });
    }
    if (action === "archiveScooter") {
      return Response.json({
        scooter: await archiveScooter(entities, user, payload),
        archived: true,
      });
    }
    ...
    return Response.json(
      { error: "The asset action could not be completed." },
      { status: 500 },
    );
  }
});
