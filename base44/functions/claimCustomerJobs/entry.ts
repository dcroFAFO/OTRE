  ...
  try {
    ...
    if (existingCustomer) {
      ...
      await base44.asServiceRole.entities.Customer.create({
        ...
        email,
        phone: user.phone || jobs[0]?.customer_phone_e164 || jobs[0]?.customer_phone || '',
        phone_e164: user.phone || jobs[0]?.customer_phone_e164 || '',
        phone_display: jobs[0]?.customer_phone_display || '',
        status: 'active',
        tags: ['customer'],
